$PBExportHeader$w_menus.srw
$PBExportComments$Ventana para el Manejo de los Menus de la Aplicacion
forward
global type w_menus from window
end type
type mle_1 from multilineedit within w_menus
end type
type st_2 from statictext within w_menus
end type
type imprimir from picturebutton within w_menus
end type
type st_1 from statictext within w_menus
end type
type st_6 from statictext within w_menus
end type
type limpiar from picturebutton within w_menus
end type
type st_5 from statictext within w_menus
end type
type salir from picturebutton within w_menus
end type
type st_9 from statictext within w_menus
end type
type st_8 from statictext within w_menus
end type
type st_7 from statictext within w_menus
end type
type eliminar from picturebutton within w_menus
end type
type grabar from picturebutton within w_menus
end type
type insertar from picturebutton within w_menus
end type
type dw_1 from datawindow within w_menus
end type
type rr_2 from roundrectangle within w_menus
end type
type rr_1 from roundrectangle within w_menus
end type
type rr_3 from roundrectangle within w_menus
end type
end forward
...
string title = "Menus de la Aplicación..."
boolean controlmenu = true
boolean minbox = true
windowtype windowtype = popup!
long backcolor = 79741120
string icon = "C:\Muñecos\ICONS\Tool2.ico"
mle_1 mle_1
st_2 st_2
imprimir imprimir
st_1 st_1
...
event open;m_principal.m_seguridad.m_operaciones.m_menús.Checked = TRUE
dw_1.settransobject(sqlca)
dw_1.retrieve()
cta_filas = dw_1.RowCount()
end event

event close;m_principal.m_seguridad.m_operaciones.m_menús.Checked = FALSE
end event
...
string text = "Ventana para el Manejo de los Menus de la Aplicación."
...
event clicked;IF dw_1.RowCount() < 1 THEN
	...
	RETURN
END IF	
w_segunda.Tag = "Generando Reporte de los Menus de la Aplicación... "+"      Fecha del Sistema... " + string(today(),"dd/mm/yyyy  hh:mm:ss")
w_segunda.SetMicrohelp(w_segunda.Tag)
Open(w_imprime_menus)
end event
...
boolean enabled = false
string text = "Menus de la Aplicación"
alignment alignment = center!
boolean focusrectangle = false
end type
...
event clicked;IF dw_1.ModifiedCount() + dw_1.DeletedCount() > 0 THEN
	...
	//**************************************************//
	String codigo_menu, descripcion_menu
	//*****************************************************************//
	...
	codigo_menu			= dw_1.GetItemString(fila,"codigo_menu")
	descripcion_menu	= dw_1.GetItemString(fila,"descripcion_menu")   
	
	//*********************************************************//
	//*** ASIGANCION DE LAS DATOS A LA FILA DEL DATAWINDOWS ***//
	//*********************************************************//
	dw_1.SetItem(fila,1,codigo_menu)
	dw_1.SetItem(fila,2,descripcion_menu)
	//*********************************************************//
...
event clicked;String codigo_menu, descripcion_menu
...
//***************************************************************************************//
Integer  net
net = MessageBox("Mensaje...","¿ Está Seguro de Eliminar este Menu... "+descripcion_menu+"("+codigo_menu+")"+" ?",Question!,YesNo!,2)

IF net = 2 THEN
	RETURN
ELSE
	IF dw_1.DeleteRow(fila) = -1 then
		Messagebox("Mensaje...","! Error al Eliminar la Fila ¡",Information!)
		dw_1.settransobject(sqlca)
		dw_1.retrieve()	
		cta_filas = dw_1.RowCount()
	ELSE
		...
		dw_1.settransobject(sqlca)
		dw_1.retrieve()	
		MessageBox("Mensaje...","! Se ha Eliminado Registro de la Báse de Datos ¡",Information!)
		cta_filas = dw_1.RowCount()
	END IF	
END IF
end event
