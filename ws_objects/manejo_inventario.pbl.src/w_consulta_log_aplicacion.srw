$PBExportHeader$w_consulta_log_aplicacion.srw
$PBExportComments$Ventana para Consutar el Log que genera la Aplicación
forward
global type w_consulta_log_aplicacion from window
end type
type cb_exportar from commandbutton within w_consulta_log_aplicacion
end type
type cb_limpiar from commandbutton within w_consulta_log_aplicacion
end type
type cb_ordenar from commandbutton within w_consulta_log_aplicacion
end type
type cb_filtrar from commandbutton within w_consulta_log_aplicacion
end type
type mle_1 from multilineedit within w_consulta_log_aplicacion
end type
type st_2 from statictext within w_consulta_log_aplicacion
end type
type imprimir from picturebutton within w_consulta_log_aplicacion
end type
type st_1 from statictext within w_consulta_log_aplicacion
end type
type st_6 from statictext within w_consulta_log_aplicacion
end type
type limpiar from picturebutton within w_consulta_log_aplicacion
end type
type st_5 from statictext within w_consulta_log_aplicacion
end type
type salir from picturebutton within w_consulta_log_aplicacion
end type
type dw_1 from datawindow within w_consulta_log_aplicacion
end type
type rr_2 from roundrectangle within w_consulta_log_aplicacion
end type
type rr_3 from roundrectangle within w_consulta_log_aplicacion
end type
type rr_1 from roundrectangle within w_consulta_log_aplicacion
end type
end forward
...
string title = "Log de la Aplicación..."
...
global w_consulta_log_aplicacion w_consulta_log_aplicacion
...
on w_consulta_log_aplicacion.create
...
on w_consulta_log_aplicacion.destroy
...
event open;m_principal.m_manejodeactivos.m_consultadellogdelaaplicación.Checked = TRUE
...
IF permiso_ventana   = "N" THEN
	...
	CLOSE(w_consulta_log_aplicacion)
	RETURN
END IF
...
event close;m_principal.m_manejodeactivos.m_consultadellogdelaaplicación.Checked = FALSE
end event
...
DO WHILE condicion = TRUE
	nombre_archivo = gb_directorio_salida+"\Log Seguridad Control Activos"
		
	IF DirectoryExists(gb_directorio_salida) THEN
	ELSE
		IF CreateDirectory(gb_directorio_salida) = -1 THEN
			MessageBox("Mensaje - Validación...","! No se Pudo Crear Carpeta "+gb_directorio_salida+" ¡",StopSign!)
			RETURN
		END IF	
	END IF
...
IF cuenta_puntos > 1 THEN
	MessageBox("Mensaje - Validación...","! El Nombre del Archivo Contiene Caracteres No Validos ¡",Exclamation!)
	RETURN
END IF	
...
dw_1.SetSort("nombre_tabla A, login A, fecha A, codigo_movimiento A, nombre_columna A")
dw_1.Sort()
dw_1.SetRedraw(true)
dw_1.GroupCalc()
end event
...
string text = "Ventana para la Consulta del Log de la Aplicación que genera las Tablas del Sistema de Activos Fijos."
...
w_segunda.SetMicrohelp(w_segunda.Tag)
Open(w_imprime_log_control_activos)
end event
...
boolean enabled = false
string text = "Consulta Log de la Aplicación de Activos Fijos"
alignment alignment = center!
boolean focusrectangle = false
end type
...
event clicked;Close(w_consulta_log_aplicacion)
...
integer height = 1364
integer taborder = 10
string dataobject = "dw_consulta_log_aplicacion"
