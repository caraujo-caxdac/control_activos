$PBExportHeader$w_responsables_activos.srw
$PBExportComments$Ventana para Definir las Personas Reponsables de los Activos
forward
global type w_responsables_activos from window
end type
type mle_1 from multilineedit within w_responsables_activos
end type
type st_2 from statictext within w_responsables_activos
end type
type imprimir from picturebutton within w_responsables_activos
end type
type st_1 from statictext within w_responsables_activos
end type
type st_6 from statictext within w_responsables_activos
end type
type limpiar from picturebutton within w_responsables_activos
end type
type st_5 from statictext within w_responsables_activos
end type
type salir from picturebutton within w_responsables_activos
end type
type st_9 from statictext within w_responsables_activos
end type
type st_8 from statictext within w_responsables_activos
end type
type st_7 from statictext within w_responsables_activos
end type
type eliminar from picturebutton within w_responsables_activos
end type
type grabar from picturebutton within w_responsables_activos
end type
type insertar from picturebutton within w_responsables_activos
end type
type dw_1 from datawindow within w_responsables_activos
end type
type rr_2 from roundrectangle within w_responsables_activos
end type
type rr_1 from roundrectangle within w_responsables_activos
end type
type rr_3 from roundrectangle within w_responsables_activos
end type
end forward
...
string title = "Responsables de los Activoss..."
...
global w_responsables_activos w_responsables_activos
...
on w_responsables_activos.create
...
on w_responsables_activos.destroy
destroy(this.mle_1)
destroy(this.st_2)
...
event open;m_principal.m_tablasbásicas.m_responsable.m_responsables.Checked = TRUE
...
event close;m_principal.m_tablasbásicas.m_responsable.m_responsables.Checked = FALSE
end event
...
string text = "Ventana para la Definición de los Responsables de los Activos de la Empresa."
...
event clicked;IF dw_1.RowCount() < 1 THEN
	...
	RETURN
END IF	
w_segunda.Tag = "Generando Reporte de los Responsables de los Activos Fijos... "+"      Fecha del Sistema... " + string(today(),"dd/mm/yyyy  hh:mm:ss")
w_segunda.SetMicrohelp(w_segunda.Tag)
Open(w_imprime_responsables_activos)
end event
...
boolean enabled = false
string text = "Responsables de los Activos"
alignment alignment = center!
boolean focusrectangle = false
end type
...
event clicked;IF dw_1.ModifiedCount() + dw_1.DeletedCount() > 0 THEN
	...
	//**************************************************//
	String clase_documento, cargo_empresa, activo_si_no
	...
	clase_documento		= dw_1.GetItemString(fila,"clase_documento")
	identificacion			= dw_1.GetItemNumber(fila,"identificacion")	
	cargo_empresa			= dw_1.GetItemString(fila,"cargo_empresa")
	...
	//*********************************************************//
	dw_1.SetItem(fila,1,clase_documento)
	dw_1.SetItem(fila,2,identificacion)
	dw_1.SetItem(fila,4,cargo_empresa)
	dw_1.SetItem(fila,5,activo_si_no)
	//*********************************************************//
...
String clase_documento, cargo_empresa, activo_si_no, descripcion_tercero
Double identificacion

clase_documento			= dw_1.GetItemString(fila,"clase_documento")
identificacion				= dw_1.GetItemNumber(fila,"identificacion")
descripcion_tercero		= dw_1.GetItemString(fila,"descripcion_tercero")
activo_si_no				= dw_1.GetItemString(fila,"activo_si_no")
...
Long cuenta_filas
//************************************************************************//
//*** BUSCA EN LA TABLA DE activos_cargo_modelo_inventario PARA VER SI EXISTE ***//
//************************************************************************//
//SELECT COUNT(*) INTO :cuenta_filas FROM activos_cargo_modelo_inventario WHERE codigo_cargo = :codigo_cargo;
//
...
//************************************************************************//
Integer  net
net = MessageBox("Mensaje - Validación...","¿ Está Seguro de Eliminar... "+descripcion_tercero+" ("+Trim(clase_documento)+&
			"-"+String(identificacion,"###,###,###,###")+")"+" ?",Question!,YesNo!,2)
...
integer height = 896
integer taborder = 10
string dataobject = "dw_responsables_activos"
...
////****************************************************//
//
IF dwo.Name = "identificacion" THEN
	IF dw_1.AcceptText() = 1 THEN
	...
	Double identificacion
	String clase_documento, descripcion_tercero
	Long cuenta_filas
	identificacion		= dw_1.GetItemNumber(row,"identificacion")
	clase_documento	= dw_1.GetItemString(row,"clase_documento")
	//********************************************************//
	//*** BUSCA SI YA EXISTE ESE CODIGO EN LA TABLA DE afp ***//
	//********************************************************//
	SELECT COUNT(*) INTO :cuenta_filas FROM terceros WHERE clase_documento = :clase_documento AND identificacion = :identificacion;
	
	IF cuenta_filas = 0 THEN
		MessageBox("Mensaje - Validación...","! La Identificación "+Trim(clase_documento)+"-"+String(identificacion,"###,###,###,###")+&
							" NO EXISTE en la Base de Datos ¡",StopSign!)
		RETURN
	ELSE
		SELECT descripcion_tercero INTO :descripcion_tercero FROM terceros WHERE clase_documento = :clase_documento AND identificacion = :identificacion;		

		dw_1.SetItem(row,3,descripcion_tercero)

		IF dw_1.AcceptText() = 1 THEN
	
		ELSE	
			MessageBox("Mensaje - Validación...","! No se Aceptaron Todos los Cambios en la Coluna de descripcion_tercero ",StopSign!)
			RETURN
		END IF		
	END IF
END IF
